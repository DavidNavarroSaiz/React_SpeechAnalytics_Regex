...
function App() {
  ...
  const [Areas_of_Interest, setAreasOfInterest] = useState([]);
...
const AnalyzeResult = async () => {
  const previousResult = previousResultRef.current;
    ...
    const apiUrl = 'http://127.0.0.1:8000/extract_info'; // Update with your API URL

    const requestBody = {
      text: previousResult,
      language: languageSelect  
      // text: "my name is david camilo navarro saiz and i live in medellin street 3#56-12 and my objective with this course is to learn new techniques and have a lot of friends, i am not very good with technology and science i am 30 years old and my email is davarros@unal.edu.co " // Assuming this is how your Python API expects the input
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        const { Name, Age, Location, Address,dates,phones, Email, Areas_of_Interest, Goals } = resultado;

        // Update state variables with extracted data
        setName((prevNames) => new Set([...prevNames, Name]));
        setName((prevPersonType) => new Set([...prevPersonType, Name]));
        setAge((prevAges) => new Set([...prevAges, Age]));
        setLocation((prevLocation) => new Set([...prevLocation, Location]));
        setAddress((prevAddresses) => new Set([...prevAddresses, Address]));
        setDateTime((prevDates) => new Set([...prevDates, dates]));
        setTelephone((prevPhone) => new Set([...prevPhone, phones]));
        setEmail((prevEmail) => new Set([...prevEmail, Email]));

        setAreasOfInterest((prevAreasOfInterest) => new Set([...prevAreasOfInterest, ...Areas_of_Interest]));
        setGoals((prevGoals) => new Set([...prevGoals, ...Goals]));
        ...
        // Now you can process the resultado object and update your state accordingly
      } else {
          ...
          <div className="card">
            <div className="card-body">
              ...
              <div className="form-group">
                ...
                <span>{Array.from(Areas_of_Interest).join(", ")}</span>
              </div>
